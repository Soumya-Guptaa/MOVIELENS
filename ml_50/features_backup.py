...
from filter_data import top_num_movies,
...
feature_dim = ml_dataset_df.shape[1]

# Reshape the ml dataset into a tensor
train_tensor = torch.tensor(ml_dataset_df.values).view(-1, top_num_movies, feature_dim)
...
# Extract feature_dim from the column size of entries
output_dim = ratings_df.shape[1]

# Reshape the ml dataset into a tensor
ratings = torch.tensor(ratings_df.values, dtype=torch.float32).view(-1, top_num_movies, output_dim)
...
# Load the ml dataset from the CSV file
True_ratings_df = pd.read_csv('avg_ratings.csv')

# Extract feature_dim from the column size of entries
output_dim = True_ratings_df.shape[1]

# Reshape the ml dataset into a tensor
True_ratings = torch.tensor(True_ratings_df.values, dtype=torch.float32).view(-1, top_num_movies, output_dim)
